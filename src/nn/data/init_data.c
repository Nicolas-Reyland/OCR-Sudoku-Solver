...
nn_DataSet nn_loadDataSet(char* data_dir_path, bool verb_mode)
{
	...
	// load the paths
	nn_Data train_data = nn_loadSingleDataInputOutput(train_input_path, train_output_path, verb_mode, "Loading 'Train' dataset");
	nn_Data test_data = nn_loadSingleDataInputOutput(test_input_path, test_output_path, verb_mode, "Loading 'Test' dataset");
	// create the dataset
...
nn_Data nn_loadSingleDataInputOutput(char* input_path, char* output_path, bool verb_mode, const char* verb_string)
{
    ...
    size_t  next_verbose_print = 0,
			next_verbose_print_step = num_tuples / 100;

    for (size_t data_index = 0; data_index < num_tuples; data_index++)
    {
        // verbose print
        // better to check now for VERBOSE, bc we don't want to enter calculate data_index == next_verbose_print
        // for nothing
