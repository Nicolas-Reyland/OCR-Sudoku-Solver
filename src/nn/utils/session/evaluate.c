...
void _nn_backPropagation(nn_Model* model, double* desired_output)
{
  ...
  for (size_t j = 0; j < output_layer->nb_nodes; j++) {
    ...
    double d_error = output_layer->nodes[j]->value - desired_output[j];
    output_layer->nodes[j]->delta_value = d_error * _nn_derivativeActivation(output_layer->nodes[j]->value, output_layer->activation);
  ...
  for (size_t i = model_layers->num_hidden_layers - 1; /* manually breaking later instead of condition here */ ; i--) {
    ...
    for (size_t j = 0; j < current_layer->nb_nodes; j++) {
      current_layer->nodes[j]->delta_value = current_layer->nodes[j]->d_value * _nn_derivativeActivation(current_layer->nodes[j]->value, current_layer->activation);
