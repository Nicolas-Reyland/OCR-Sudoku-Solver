...
void _nn_backPropagation(nn_Model* model, double* desired_output)
{
  ...
  for (size_t j = 0; j < layers[num_layers - 1]->num_nodes; j++) {
    ...
    layers[num_layers-1]->nodes[j]->d_raw_value = d_error * _nn_derivativeActivation(layers[num_layers-1]->nodes[j]->value, layers[num_layers-1]->activation);
    //
  ...
  for (size_t i = num_layers - 2; i > 0; i--) {
    for (size_t j = 0; j < layers[i]->num_nodes; j++) {
      layers[i]->nodes[j]->d_raw_value = layers[i]->nodes[j]->d_value;
