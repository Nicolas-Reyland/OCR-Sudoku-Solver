...
size_t findgrid(CCTuple *histo, size_t histo_size, size_t* coords[], SDL_Surface** surf[])
{
    ...
    do
    {
        ...
        *croppedsegmap = cut_segmap(*segmap, *top, *bottom, *left, *right);
        ...
        size_t area = (size_t)(abs((int)*bottomrightX - (int)*topleftX))*abs(((int)*bottomleftY - (int)*toprightY)) 
            + abs(((int)*toprightX - (int)*bottomleftX))*abs(((int)*bottomrightY - (int)*topleftY));
